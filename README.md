...
[MTF Dashboard Bull 2026-05-01 23.40.12.png](https://github.com/WiseManAlgo/MTF-Confluence-Dashboard/blob/3e2f982541e0650b09170be231af916051772548/MTF_Dasboard_Bull_%202026-05-01_23.40.12.png)

**BEAR Market Screenshot**
[MTF Dashboard Bear 2025-05-01 23.25.34.png](https://github.com/WiseManAlgo/MTF-Confluence-Dashboard/blob/1a04af177cdf66660e2ffa9173476bff0cfe033c/MTF_Dashboard%20_Bear_2026-05-01_23.25.34.png)
...
- **Score range is now a clean ±20.** Weights are normalized before scoring, so the WScore range is always fixed at ±20 regardless of how you adjust TF weights. A new "Score Scale" input (default: 10) controls this directly.
- **Stricter signal threshold: default raised to ±10, strictly greater than.** A score of exactly ±10 does not fire — the market must show clearer confluence. On the ±20 scale this is equivalent to the old ±7 on the previous ±14 scale, but with a harder edge.
...
- **Volume filter now raises the threshold instead of blocking signals.** Low volume no longer kills a strong signal outright — it raises the trigger threshold by a configurable multiplier (default ×1.3). The Vol column shows the active multiplier when engaged.
...
This is what makes this indicator different from every other MTF tool on the platform: ❌ Everyone else uses: RSI > 55 = bull, RSI < 45 = bear (fixed forever, becomes long-only in bull markets) ✅ We use: RSI above its own EMA signal line → +1; below signal line → -1
...
Weights are normalized so the final score always ranges from **-20 (extreme bearish)** to **+20 (extreme bullish)**. Every single weight and parameter is fully adjustable in the settings.
...
- Crank it up to 14 → Fewer signals, higher conviction, misses some moves
...
**⚠️ Important:** Signal arrows only fire for Symbol 1. By default, Symbol 1 is always your base chart ticker — switch charts and it follows automatically. Symbols 2 and 3 in the dashboard are for monitoring only — they show scores but do not generate arrows or alerts.
...
- **WScore**: The final weighted total for that symbol. Turns bright green above +10, bright red below -10. You can see if a signal has fired in 0.1 seconds.
- **Vol**: Status of the optional volume filter (OFF by default).

**About That Volume Filter**

The volume filter is **OFF BY DEFAULT**, with a big fat disclaimer:

This filter will raise the signal threshold during stock midday lulls and crypto weekends. Only enable it if you trade assets with clean, consistent volume data, and you've tested it yourself. When active and volume is low, the trigger threshold is multiplied by the Vol Multiplier setting (default ×1.3) — strong signals can still fire, weak borderline ones are filtered out.
